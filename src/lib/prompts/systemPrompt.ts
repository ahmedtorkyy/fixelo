...
- Use only REAL cmdlets that exist in Windows. Here are proven real cmdlets by category:
  ...
  · Language & Input: For keyboard layouts and input languages, use Get-WinUserLanguageList, Set-WinUserLanguageList. To add a language: $list = Get-WinUserLanguageList; $list.Add('es-ES'); Set-WinUserLanguageList $list. IMPORTANT: The InputMethodList property on each WinUserLanguage object can be $null. Always check for null before calling .Clear() or .Add() on it. Example: if ($lang.InputMethodList -ne $null) { ... }. To set a specific keyboard layout, use $lang.InputMethodList.Add('0409:00000409') where the format is "localeID:keyboardLayoutID". Never use New-WinUserLanguageList without calling Set-WinUserLanguageList — it creates a list object but does NOT apply it.
